...
//LEGENDS
import DNGLegendSrc from "/assets/Icon/legend/DNG_legend.png";

import TransitionsModal from "../Attribute/dng_attr.jsx"
...
const MapDng = () => {
  ...
  const [openAttr, setOpenAttr] = useState(false);
    ...
    const handleOpenAttr = () => setOpenAttr(true);
    const handleCloseAttr = () => setOpenAttr(false);
  ...
  useEffect(() => {
    ...
    const DNGLegend = document.getElementById("legend");
        if (DNGLegend) {
        DNGLegend.src = DNGLegendSrc; // Use the correct image URL
    ...
    script.onload = () => {
      ...
      map.Event.bind(sphere.EventName.Ready, async function () {
        ...
        if (isPM25Checked) { 
        const dngMap = new sphere.Layer("0", {
            type: window.sphere.LayerType.WMS,
            
            url: 'https://gistdaportal.gistda.or.th/imagedata/rest/services/GISTDA_LifeD/dengue/ImageServer/exportImage?f=image&bbox=11187934.956044678%2C1550754.4298496544%2C11192826.925854929%2C1555646.3996599056&bboxSR=102100&size=&imageSR=102100&time=&format=jpgpng&pixelType=S16&noData=',
            zoomRange: { min: 1, max: 15 },
            zIndex: 5,
            opacity: 0.75
        });
        
        map.Layers.add(dngMap);
        setPm25wmsLayer(dngMap);
        ...
        if (map && map.Ui && map.Ui.Geolocation) {
            map.goTo({ center: { lon: 102.10759622496983, lat: 14.97385471711737 }, zoom: 8 });    
  ...
  const toggleSidebar = () => {
    ...
    // if (!openSidebar) setopenDashboard(false);
  };
    ...
    <>
      ...
      >
        <>
          ...
          <TransitionsModal open={openAttr} handleClose={handleCloseAttr} />
        </>
      </div>
    </>
  );
