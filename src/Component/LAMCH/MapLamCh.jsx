...
//LEGENDS
import pmLegendSrc from "/assets/Icon/legend/pm_lam.png";
import rainLegendSrc from "/assets/Icon/legend/rain_lam.png";
import tempLegendSrc from "/assets/Icon/legend/temp_lam.png";
...
const MapLamCh = () => {
  ...
  const handleToggle3D = (layerKey) => {
    ...
    const layer3D = new sphere.Layer("Laem_Chabang_BF_Layer3DToFea", {
      type: sphere.LayerType.I3S,
      url: "https://gistdaportal.gistda.or.th/arcgis/rest/services/Hosted/Laem_Chabang_3D/SceneServer/layers/0",
      onTilesetLoad: () => {
        map.goTo({
          center: { lon: 100.9175, lat: 13.0928 },
          zoom: 15,
          pitch: 60,
        });
      },
    ...
    setLayers((prev) => {
      ...
      if (isCurrentlyVisible) {
        map.Layers.remove(layer3D);
        console.log("layer3D Removed");
      } else {
        map.Layers.add(layer3D);
        console.log("layer3D Added");
      }
    ...
    script.onload = () => {
      ...
      map.Event.bind(sphere.EventName.Ready, async function () {
        ...
        if (map && map.Ui && map.Ui.Geolocation) {
          map.goTo({
            center: { lon: 101.25049566240266, lat: 13.005572401881508 },
            zoom: 8,
          });
  ...
  const histData = () => {
    ...
    setActiveScenario("histData");
    setdisplaySlide("");
    setdisplayDate("none");
  };

  const pmLamSelect = () => {
    setSelectFunction(() => pmLamSelect);
    ...
    //////// lastedData TIME + 4 1)2029 2)2033 3)2037 4)2042 5)2046 ////////

    // const newWmsLayer = new window.sphere.Layer(`0`, {
    //   type: window.sphere.LayerType.WMS,
    //   url: `https://gistdaportal.gistda.or.th/data/services/Microclimate/Rainfall_Monthly/MapServer/WMSServer`,
    //   // layers: `lastedData_Korat_26_50:UTFVI_Transparent`,
    //   format: "image/png",
    //   transparent: true,
    //   zoomRange: { min: 1, max: 15 },
    //   opacity: 0.6,
    ...
    //   title: "PM25_03.tif",
    //   type: window.sphere.LayerType.WMS,
    //   url: `https://gistdaportal.gistda.or.th/data/services/Microclimate/PM25_Monthly/MapServer/WMSServer`,
    //   layers: '9',
    //   format: "image/png",
    //   transparent: true,
    //   zoomRange: { min: 1, max: 15 },
    ...
    const newWmsLayer = new window.sphere.Layer('pm25Layer', {
      title: "PM25_03.tif",
      type: window.sphere.LayerType.WMS,
      url: 'https://gistdaportal.gistda.or.th/data/services/Microclimate/PM25_Monthly/MapServer/WMSServer',
      layers: '9',
    ...
    const match = pairedYears.find(
      (pair) => selectedYear45 >= pair.start && selectedYear45 <= pair.end
    );
    ...
    setrcpLayer("lastedData");

    const lastedDataData = async () => {
      try {
        const res = await axios.get(
          `https://gistdaportal.gistda.or.th/imagedata/rest/services/GISTDA_LifeD/lastedData_Korat_26_50/ImageServer/identify`,
          {
            params: {
        ...
        const updateDate =
          res.data.catalogItems.features[0].attributes.UpdateDate;
    ...
    setsliderStep(step);
    handleSlide("lastedData");
  };

  const rainLamSelect = () => {
    setSelectFunction(() => rainLamSelect);
    ...
    // const newWmsLayer = new window.sphere.Layer(`lastedData_Korat_26_50`, {
    //   type: window.sphere.LayerType.WMS,
    //   url: `https://gistdaportal.gistda.or.th/imagedata/services/GISTDA_LifeD/lastedData_Korat_26_50/ImageServer/WMSServer?&time=${selectedYear45}`,
    //   layers: `lastedData_Korat_26_50:UTFVI_Transparent`,
    //   format: "image/png",
    //   transparent: true,
    //   zoomRange: { min: 1, max: 15 },
  ...
  const tempLamSelect = () => {
    setSelectFunction(() => tempLamSelect);
    ...
    const newWmsLayer = new window.sphere.Layer(`0`, {
      type: window.sphere.LayerType.WMS,
      url: `https://gistdaportal.gistda.or.th/data/services/Microclimate/Temp_Monthly/MapServer/WMSServer`,
      // layers: `lastedData_Korat_26_50:UTFVI_Transparent`,
      format: "image/png",
      transparent: true,
      zoomRange: { min: 1, max: 15 },
  ...
  const [selectFunction, setSelectFunction] = useState(() => pmLamSelect);
  ...
  useEffect(() => {
    if (
      ...
      // console.log("pmLamSelect กำลังจะรัน...");
      lastedData();
    }
  ...
  const handleSlide = (event, newvalue) => {
    console.log("==== Slide ====",event, newvalue);
  }

  const handlePlay = (event, newvalue) => {
    console.log("==== Play ====",event, newvalue);
  }
  ...
  //     setIsPlaying(false);
  //   } else {
  //     const step = rcpLayer === "lastedData" ? 4 : 5;
  //     const maxYear = rcpLayer === "lastedData" ? 2050 : 2047;
  //     const baseYear = rcpLayer === "lastedData" ? 2028 : 2026;
      ...
      >
        <>
          ...
          >
            ...
            >
              {/* <Button variant="contained" onClick={() => pmLamSelect()}>
                lastedData
              </Button>
              <Button variant="contained" onClick={() => histData()}>
                histData
              </Button> */}
                ...
                >
                  <Box sx={{ p: 2 }}>
                    ...
                    >
                      ...
                      <Switch
                        checked={layers.building3D}
                        onChange={() => handleToggle3D("building3D")}
                        size="small"
                        sx={{
                  ...
                  <Box sx={{ p: 2, borderBottom: "2px solid #f5f5f5" }}>
                    {tabIndex === 0 && (
                      ...
                      >
                        ...
                        <Typography
                          variant="body1"
                          sx={{ color: "#7091ba", display: `${displayDate}` }}
                        >
                          อัปเดตข้อมูล PM2.5 ล่าสุด วันจันทร์ ที่ 30 ธันวาคม
                          พ.ศ.2562
                        </Typography>
                      </Box>
                    )}
                    {tabIndex === 1 && (
                      ...
                      >
                        ...
                        <Typography
                          ...
                          sx={{ color: "#7091ba", display: `${displayDate}` }}
                        >
                          อัปเดตข้อมูลฝน ล่าสุด วันจันทร์ ที่ 30 ธันวาคม
                          พ.ศ.2562
                        </Typography>
                      </Box>
                    )}
                    {tabIndex === 2 && (
                      ...
                      >
                        ...
                        <Typography
                          ...
                          sx={{ color: "#7091ba", display: `${displayDate}` }}
                        >
                          อัปเดตข้อมูลอุณหภูมิ ล่าสุด วันพุธ ที่ 28 เมษายน
                          พ.ศ.2562
                        </Typography>
                      </Box>
                    )}
